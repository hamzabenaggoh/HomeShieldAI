...
# Get model
rf = load('/Users/hamzabenaggoun/Desktop/HomeShieldAI/anomaly_detection/model/models/clf_rf.joblib')
...
selected_features = load('/Users/hamzabenaggoun/Desktop/HomeShieldAI/anomaly_detection/model/features/selected_features.joblib')

# Get the scaler
scaler = load('/Users/hamzabenaggoun/Desktop/HomeShieldAI/anomaly_detection/model/scalers/scaler.joblib')
...
# Filter rows where prediction is not 0 (attack detected)
attack_indices = np.where(predictions != 0)[0]

if len(attack_indices) > 0:
    attack_metadata = og_df.iloc[attack_indices]
    attack_metadata["Label"] = predictions[attack_indices]
    print("\nDetected Attacks (Metadata):")
    print(attack_metadata)

    # Save to a CSV file if needed
    # attack_metadata.to_csv("detected_attacks.csv", index=False)
    # print("\nAttack metadata saved to 'detected_attacks.csv'")
else:
    print("\nNo attacks detected.")
