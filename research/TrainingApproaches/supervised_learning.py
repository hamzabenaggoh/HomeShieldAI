...
# from scipy import interp    #TODO: Dont need i guess
...
# data = arff.loadarff('./CICIDS2017_dataset.csv')
...
# dataset = dataset.drop(['Flow_ID', '_Source_IP', '_Destination_IP', '_Timestamp'], axis = 1) #TODO: Not needed?
...
max_count = 1501
# iBenign = 20000   #TODO: Change back later
benign_count = 20000
iDDoS = 0
iBot = 0
iDoS_slowloris = 0
...
# for i in range(0,948995):     #TODO: Optimize this later! To use pandas filtering
#     # print(i)
#     try:
...
# Create a filtered dataset by sampling up to max_count rows for each attack type
filtered_dataset = pd.concat([
    dataset[dataset['Label'] == 'BENIGN'].head(benign_count),
    dataset[dataset['Label'] == 'DDoS'].head(max_count),
    dataset[dataset['Label'] == 'Bot'].head(max_count),
    dataset[dataset['Label'] == 'DoS slowloris'].head(max_count),
    dataset[dataset['Label'] == 'DoS Slowhttptest'].head(max_count),
    dataset[dataset['Label'] == 'DoS Hulk'].head(max_count),
    dataset[dataset['Label'] == 'DoS GoldenEye'].head(max_count),
    dataset[dataset['Label'] == 'Heartbleed'].head(max_count),
    dataset[dataset['Label'] == 'FTP-Patator'].head(max_count),
    dataset[dataset['Label'] == 'SSH-Patator'].head(max_count),
    dataset[dataset['Label'] == 'Web Attack – Brute Force'].head(max_count),
    dataset[dataset['Label'] == 'Web Attack – XSS'].head(max_count),
    dataset[dataset['Label'] == 'Web Attack – Sql Injection'].head(max_count),
    dataset[dataset['Label'] == 'Infiltration'].head(max_count),
])
...
filtered_dataset.to_csv('sample_dataset.csv', index=None, header=True)

print("Filtered dataset created and saved as 'sample_dataset.csv'.")


# # Creating X and Y from the dataset
from sklearn import preprocessing
le = preprocessing.LabelEncoder()
Y_class = filtered_dataset['Label'].values  # 'Label' column
le.fit(Y_class)
